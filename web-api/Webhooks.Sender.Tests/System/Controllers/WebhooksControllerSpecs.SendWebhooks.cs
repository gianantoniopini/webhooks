...
namespace Webhooks.Sender.Tests.System.Controllers
{
    public partial class WebhooksControllerSpecs
    {
        ...
        [Fact]
        public async Task SendWebhooks_IfAllRequestsFail_ShouldStillReturnOk()
        {
            // Arrange
            var webhooks = new List<Webhook>
            {
            ...
            _httpMessageHandler.SetupRequest(HttpMethod.Post, webhooks[1].PayloadUrl).ReturnsAsync(new HttpResponseMessage(HttpStatusCode.InternalServerError), TimeSpan.FromMilliseconds(500));
            _httpMessageHandler.SetupRequest(HttpMethod.Post, webhooks[2].PayloadUrl).ReturnsAsync(new HttpResponseMessage(HttpStatusCode.BadRequest), TimeSpan.FromMilliseconds(150));

            // Act
            var result = await _controller.SendWebhooks();
